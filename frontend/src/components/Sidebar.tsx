...
export default function Sidebar() {
  ...
  const { isMobileMenuOpen, toggleMobileMenu, closeMobileMenu, isCollapsed, toggleCollapsed } = useSidebar();
  ...
  const renderMenuItem = (item: MenuItem, isChild: boolean = false) => {
      ...
      return (
        <div key={item.name}>
          <button
            ...
            className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors w-full relative
              ${isActive
    ...
    return (
      <Link
        ...
        title={isCollapsed && !isChild ? item.name : undefined}
      >
        {isActive && !isChild && (
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full bg-white shadow-xl z-40 transform transition-all duration-300 ease-out border-r border-gray-100
      ...
      >
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="flex items-center h-16 border-b border-gray-100 gap-3 px-5 justify-between">
            <div className="flex items-center gap-3 min-w-0">
              ...
              {!isCollapsed && (
                <div className="min-w-0">
                  <h1 className="text-lg font-bold text-primary-600 whitespace-nowrap truncate">TITIPIN</h1>
            ...
            >
              <ChevronLeft size={18} className={`text-gray-500 transition-transform duration-300 ${isCollapsed ? 'rotate-180' : ''}`} />
            </button>
          </div>
          ...
          <div className="p-3 border-t border-gray-100">
            ...
            {userRole && isCollapsed && (
              <div className="mb-3 px-3 flex justify-center" title={ROLE_DISPLAY_NAMES[userRole] || userRole}>
                <div className={`p-1.5 rounded-full ${ROLE_COLORS[userRole] || 'bg-gray-100 text-gray-800'}`}>
                  <Shield size={16} />
                </div>
              </div>
            )}
